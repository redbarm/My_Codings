...
LeptonThread::LeptonThread() : QThread()
{
	...
	spiSpeed = 18 * 1000 * 1000; // SPI bus speed 18MHz
...
void LeptonThread::run()
{
	...
	while(true) {
		...
		for(int j=0;j<PACKETS_PER_FRAME;j++) {
			...
			if ((typeLepton == 3) && (packetNumber == 20)) {
				...
				if ((segmentNumber < 1) || (4 < segmentNumber)) {
					log_message(10, "[ERROR] Wrong segment number " + patch::to_string(segmentNumber));
					break;
				}
			}
		}
		if(resets >= 30) {
			log_message(3, "done reading, resets: " + patch::to_string(resets));
		}
		...
		if (typeLepton == 3) {
			if ((segmentNumber < 1) || (4 < segmentNumber)) {
				...
				if ((n_wrong_segment % 12) == 0) {
					log_message(5, "[WARNING] Got wrong segment number continuously " + patch::to_string(n_wrong_segment) + " times");
				}
				continue;
			}
			if (n_wrong_segment != 0) {
				log_message(8, "[WARNING] Got wrong segment number continuously " + patch::to_string(n_wrong_segment) + " times [RECOVERED] : " + patch::to_string(segmentNumber));
				n_wrong_segment = 0;
			}
			...
			for(int i=0;i<FRAME_SIZE_UINT16;i++) {
				...
				if (valueFrameBuffer == 0) {
					...
					if ((n_zero_value_drop_frame % 12) == 0) {
						log_message(5, "[WARNING] Found zero-value. Drop the frame continuously " + patch::to_string(n_zero_value_drop_frame) + " times");
					}
					break;
				}
		...
		if (n_zero_value_drop_frame != 0) {
			log_message(8, "[WARNING] Found zero-value. Drop the frame continuously " + patch::to_string(n_zero_value_drop_frame) + " times [RECOVERED]");
			n_zero_value_drop_frame = 0;
		}
